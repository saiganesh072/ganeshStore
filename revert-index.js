...
// Reverse replacements to restore old products on index.html
const replacements = [
...
replacements.forEach(rep => {
  html = html.split(rep.newUrl).join(rep.oldUrl);
  html = html.split(rep.newImg).join(rep.oldImg);
  html = html.split(rep.newName).join(rep.oldName);
});

fs.writeFileSync('index.html', html, 'utf-8');
console.log('Reverted index.html back to old products!');
