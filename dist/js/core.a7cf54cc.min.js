...
words[(((len + 8) >>> 6) << 4) + 15] = len * 8;for (var chunk = 0; chunk < words.length; chunk += 16) {
var W = new Array(80);
for (var t = 0; t < 16; t++) W[t] = words[chunk + t] || 0;
for (var t = 16; t < 80; t++) W[t] = rotl(W[t - 3] ^ W[t - 8] ^ W[t - 14] ^ W[t - 16], 1);var a = H[0], b = H[1], c = H[2], d = H[3], e = H[4];
for (var t = 0; t < 80; t++) {
...
var timeStep = Math.floor(epoch / 30);var timeHex = ('0000000000000000' + timeStep.toString(16)).slice(-16);
var timeBytes = this.hexToBytes(timeHex);
var keyHex = this.base32tohex(secret);
var keyBytes = this.hexToBytes(keyHex);var hmac = this.hmacSha1(keyBytes, timeBytes);
var offset = hmac[hmac.length - 1] & 0x0f;
var binary = ((hmac[offset] & 0x7f) << 24) |
((hmac[offset + 1] & 0xff) << 16) |
((hmac[offset + 2] & 0xff) << 8) |
(hmac[offset + 3] & 0xff);var otp = (binary % 1000000).toString();
...
if (!user) throw new Error('You must be signed in to configure 2-Factor Authentication.');var isValid = TOTP.verifyToken(secret, token);
if (!isValid) {
...
var user = this.getCurrentUser();
if (!user) throw new Error('You must be signed in.');if (user.two_factor_secret && token) {
...
if (!user.two_factor_secret) throw new Error('2FA is not configured for this account.');var isValid = TOTP.verifyToken(user.two_factor_secret, token);
if (!isValid) {
...
document.cookie = "CustomerNumber=" + encodeURIComponent(user.phone || user.phone_number || '') + "; path=/;";await BackendService.cart.fetchCartFromCloud(user.id);
await BackendService.wishlist.fetchWishlistFromCloud(user.id);if (window.trackACDLUserLogin) {
...
var password = userData.password || 'password123';var users = Storage.get('ganeshStore_registered_users', []);
var existing = users.find(function (u) { return u.email === email || (phone && u.phone === phone); });if (existing) {
...
var users = Storage.get('ganeshStore_registered_users', []);
var user = users.find(function (u) { return u.email === cleanEmail; });if (!user || user.password !== password) {
if (BackendService._isCloudAvailable && BackendService._client) {
try {
...
var cleanName = (name || 'Guest User').trim();var users = Storage.get('ganeshStore_registered_users', []);
var user = users.find(function (u) { return u.phone === cleanPhone; });if (!user) {
...
document.cookie = "CustomerName=" + encodeURIComponent(user.name || user.full_name) + "; path=/;";
document.cookie = "CustomerNumber=" + encodeURIComponent(user.phone || user.phone_number || '') + "; path=/;";await BackendService.cart.fetchCartFromCloud(user.id);
await BackendService.wishlist.fetchWishlistFromCloud(user.id);return user;
},
logout: function () {
...
if (!currentUser) throw new Error('No user logged in.');var updated = Object.assign({}, currentUser, profileUpdates);
...
var perk = 'Standard 1x Points & Free Shipping Over $100';if (points >= 2000) {
tier = 'Platinum';
nextTier = 'Max Tier';
pointsToNext = 0;
...
Storage.set('cart', cartItems);this.updateBadges();
this.syncCartToCloud();if (typeof window.updateFreeShippingProgressBar === 'function') {
window.updateFreeShippingProgressBar();
}$(document).trigger('cartUpdated');
},addToCart: function (item) {
var cart = this.getCart();
var qty = parseInt(item.quantity, 10) || 1;var existing = cart.find(function (i) {
...
}this.saveCart(cart);if (window.trackACDLAddToCart) {
...
return tot + (parseInt(i.quantity, 10) || 1);
}, 0);$('.js-show-cart').each(function () {
$(this).attr('data-notify', totalQuantity);
});
},syncCartToCloud: async function () {
var user = BackendService.auth.getCurrentUser();
if (!user || !BackendService._isCloudAvailable || !BackendService._client) return;var cart = this.getCart();
try {
...
$(document).trigger('wishlistUpdated', { added: added, item: item });if (window.trackACDLWishlistToggle) {
...
var user = BackendService.auth.getCurrentUser();
if (!user || !BackendService._isCloudAvailable || !BackendService._client) return;var wishlist = this.getWishlist();
try {
...
var localReviews = Storage.get(cacheKey, []);if (BackendService._isCloudAvailable && BackendService._client) {
try {
var res = await BackendService._client.from('reviews')
.select('*')
.eq('product_name', cleanName)
.order('created_at', { ascending: false });if (res.data && res.data.length > 0) {
...
var cacheKey = 'reviews_' + cleanName.replace(/\s+/g, '_');
var currentReviews = await this.getProductReviews(cleanName);var newRev = {
...
var trackingNo = 'GS-TRK-' + Math.floor(10000000 + Math.random() * 90000000);var estDate = new Date();
estDate.setDate(estDate.getDate() + (orderPayload.shipping_method === 'Express Delivery' ? 2 : 4));var orderRecord = {
order_id: orderId,
...
BackendService.cart.saveCart([]);return orderRecord;
},getUserOrders: async function (userIdOrEmail) {
var localOrders = Storage.get('order_history', []);if (BackendService._isCloudAvailable && BackendService._client && userIdOrEmail) {
try {
...
var order = orders.find(function (o) { return o.order_id === orderId || o.tracking_number === orderId; });if (!order) {
return {
found: false,
...
var elapsedHours = (Date.now() - createdTime) / (1000 * 60 * 60);var currentStep = 2;
var statusText = 'In Processing & Quality Check';if (elapsedHours > 48) {
currentStep = 5;
...
Storage.set('newsletter_subscribers', subs);if (BackendService._isCloudAvailable && BackendService._client) {
try {
...
var subject = messageData.subject || 'Customer Inquiry';if (!name || !email || !message) {
...
Storage.set('contact_messages', messages);if (BackendService._isCloudAvailable && BackendService._client) {
try {
...
var numSubtotal = parseFloat(subtotal) || 0;if (BackendService._isCloudAvailable && BackendService._client) {
try {
...
var cleanCode = (code || '').trim().toUpperCase();
var numSubtotal = parseFloat(subtotal) || 0;var coupon = ACTIVE_COUPONS[cleanCode];
if (!coupon) {
if (window.trackACDLPromoCode) {
...
BackendService.init();
window.BackendService = BackendService;})(window);
;// 3. Dynamic header updates ("My Account" -> Custom user dropdown)const firebaseConfig = {apiKey: "YOUR_API_KEY_HERE",authDomain: "YOUR_AUTH_DOMAIN_HERE",projectId: "YOUR_PROJECT_ID_HERE",storageBucket: "YOUR_STORAGE_BUCKET_HERE",messagingSenderId: "YOUR_MESSAGING_SENDER_ID_HERE",appId: "YOUR_APP_ID_HERE"};const hasRealFirebaseConfig =firebaseConfig.apiKey &&firebaseConfig.apiKey !== "YOUR_API_KEY_HERE" &&firebaseConfig.apiKey.trim() !== "";let db = null;if (hasRealFirebaseConfig) {try {firebase.initializeApp(firebaseConfig);db = firebase.firestore();console.log("🔥 Firebase initialized successfully!");} catch (error) {console.error("❌ Failed to initialize Firebase:", error);}} else {console.log("ℹ️ Running in Premium Mock Auth mode. (Edit js/auth.js to connect real Firebase)");}const dropdownStyle = document.createElement('style');dropdownStyle.innerHTML = `.auth-dropdown {position: relative;display: inline-block;}.auth-dropdown-menu {display: none;position: absolute;right: 0;top: 100%;background-color: #ffffff;min-width: 160px;box-shadow: 0px 8px 16px 0px rgba(0,0,0,0.1);z-index: 9999;border-radius: 8px;border: 1px solid #eeeeee;overflow: hidden;animation: dropdownFadeIn 0.25s ease;}.auth-dropdown-menu a {color: #333333 !important;padding: 12px 16px;text-decoration: none;display: block;font-size: 13px;font-weight: 500;transition: background-color 0.2s ease, color 0.2s ease;}.auth-dropdown-menu a:hover {background-color: #f8f9fa;color: #717fe0 !important;}.auth-dropdown:hover .auth-dropdown-menu {display: block;}@keyframes dropdownFadeIn {from { opacity: 0; transform: translateY(10px); }to { opacity: 1; transform: translateY(0); }}`;document.head.appendChild(dropdownStyle);function getCookieValue(name) {let result = document.cookie.match("(^|[^;]+)\\s*" + name + "\\s*=\\s*([^;]+)");return result ? decodeURIComponent(result.pop()) : "";}function syncUserProfileWithSupabase(uid, name, email) {if (window.supabaseClient) {console.log("🔄 Syncing user profile with Supabase...", email);return window.supabaseClient.from('profiles').select('*').eq('email', email).then(response => {let profile = response.data && response.data[0];if (profile) {console.log("✅ Supabase profile loaded:", profile);localStorage.setItem('userProfile', JSON.stringify(profile));localStorage.setItem('user', 'loggedin');return profile;} else {return window.supabaseClient.from('profiles').select('*').eq('id', uid).then(idResponse => {let idProfile = idResponse.data && idResponse.data[0];if (idProfile) {console.log("✅ Supabase profile loaded by ID:", idProfile);localStorage.setItem('userProfile', JSON.stringify(idProfile));localStorage.setItem('user', 'loggedin');return idProfile;} else {// Profile doesn't exist, create a new record!const newProfileId = (uid && uid.includes('-') && uid.length === 36) ? uid : (typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID() : 'usr_' + Date.now());const newProfile = {id: newProfileId,full_name: name || email.split('@')[0],email: email,phone_number: "1234567890",loyalty_tier: "Bronze",loyalty_points: 100,two_factor_enabled: false,two_factor_secret: null};return window.supabaseClient.from('profiles').insert(newProfile).then(insertRes => {if (insertRes.error) {console.warn("⚠️ Supabase email insert failed (old schema without email column), trying fallback profile...", insertRes.error);const fallbackProfile = {id: newProfileId,full_name: newProfile.full_name,phone_number: "1234567890"};return window.supabaseClient.from('profiles').insert(fallbackProfile).then(() => {localStorage.setItem('userProfile', JSON.stringify(fallbackProfile));localStorage.setItem('user', 'loggedin');return fallbackProfile;});}localStorage.setItem('userProfile', JSON.stringify(newProfile));localStorage.setItem('user', 'loggedin');return newProfile;});}});}}).catch(err => {console.error("❌ Supabase sync error:", err);const localProfile = { id: uid, full_name: name, email: email, phone_number: "1234567890", loyalty_tier: "Bronze", loyalty_points: 100, two_factor_enabled: false };localStorage.setItem('userProfile', JSON.stringify(localProfile));localStorage.setItem('user', 'loggedin');return localProfile;});} else {const localProfile = { id: uid, full_name: name, email: email, phone_number: "1234567890" };localStorage.setItem('userProfile', JSON.stringify(localProfile));localStorage.setItem('user', 'loggedin');return Promise.resolve(localProfile);}}document.addEventListener("DOMContentLoaded", () => {if (hasRealFirebaseConfig) {firebase.auth().onAuthStateChanged((user) => {if (user) {console.log("👤 User logged in (Firebase):", user.email);updateHeaderUI(user);} else {console.log("👤 No user logged in (Firebase)");updateHeaderUI(null);}});} else {const mockUser = getMockUser();if (mockUser) {console.log("👤 User logged in (Mock):", mockUser.email);updateHeaderUI(mockUser);} else {console.log("👤 No user logged in (Mock)");updateHeaderUI(null);}}});function updateHeaderUI(user) {const accountLinks = findMyAccountLinks();if (user) {const displayName = user.displayName || user.email.split('@')[0];const userUid = user.uid || "mock-uid-123";document.cookie = "CustomerName=" + encodeURIComponent(displayName) + "; path=/";document.cookie = "CustomerNumber=" + encodeURIComponent(userUid) + "; path=/";localStorage.setItem("user", "loggedin");}accountLinks.forEach(link => {if (user) {const displayName = user.displayName || user.email.split('@')[0];const initials = displayName.substring(0, 2).toUpperCase();link.href = "signin.html";link.classList.add("logged-in");link.innerHTML = `<span class="user-avatar-badge">${initials}</span><div class="header-account-dropdown"><div class="header-account-dropdown-greeting"><span class="user-welcome">Welcome back,</span><span class="user-name">${displayName}</span></div><div class="header-account-dropdown-links"><a href="wishlist.html"><i class="zmdi zmdi-favorite-outline"></i> My Wishlist</a><a href="shoping-cart.html"><i class="zmdi zmdi-shopping-cart"></i> My Cart</a><a href="#" class="js-logout-trigger logout-btn-dropdown"><i class="zmdi zmdi-power"></i> Log Out</a></div></div>`;} else {link.href = "signin.html";link.classList.remove("logged-in");link.innerHTML = `<i class="zmdi zmdi-account js-auth-icon" style="font-size: 26px;"></i>`;}});if (window.location.pathname.indexOf('/signin.html') !== -1) {const accountNameEl = document.querySelector('.AccountName');const accountNumberEl = document.querySelector('.AccountNumber');const logoutAvatarEl = document.querySelector('.js-logout-avatar');const loginMain = document.getElementsByClassName("Loginmain")[0];const logoutMain = document.getElementsByClassName("Logoutmain")[0];if (user) {const displayName = user.displayName || user.email.split('@')[0];const userUid = user.uid || "mock-uid-123";if (accountNameEl) accountNameEl.textContent = displayName;if (accountNumberEl) accountNumberEl.textContent = userUid;if (logoutAvatarEl) logoutAvatarEl.textContent = displayName.substring(0, 2).toUpperCase();if (loginMain) loginMain.style.display = "none";if (logoutMain) logoutMain.style.display = "block";} else {if (accountNameEl) accountNameEl.textContent = "Guest";if (accountNumberEl) accountNumberEl.textContent = "Not Logged In";if (logoutAvatarEl) logoutAvatarEl.textContent = "U";if (loginMain) loginMain.style.display = "block";if (logoutMain) logoutMain.style.display = "none";}}if (typeof loginfun === "function") {loginfun();}}function findMyAccountLinks() {const header = document.querySelector("header");if (!header) return [];const matches = header.querySelectorAll(".js-auth-link-container");if (matches.length > 0) {return Array.from(matches);}const fallbackLinks = header.querySelectorAll(".js-auth-link, a");const result = [];fallbackLinks.forEach(link => {const text = link.textContent.trim().toLowerCase();if (text === "sign in" ||text === "my account" ||text === "login" ||link.classList.contains("js-auth-link") ||link.classList.contains("js-auth-link-container")) {result.push(link);}});return result;}document.addEventListener("click", function(e) {if (e.target && e.target.classList.contains("js-logout-trigger")) {logoutUser(e);}});$(document).on('click', '.js-auth-link-container', function(e) {if ($(window).width() < 992 && $(this).hasClass('logged-in')) {e.preventDefault();e.stopPropagation();$(this).find('.header-account-dropdown').toggleClass('show-mobile');}});$(document).on('click', function(e) {if (!$(e.target).closest('.js-auth-link-container').length) {$('.header-account-dropdown').removeClass('show-mobile');}});function logoutUser(e) {if (e) e.preventDefault();document.cookie = "CustomerName=; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC;";document.cookie = "CustomerNumber=; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC;";localStorage.setItem("user", "loggedout");localStorage.removeItem('userProfile');localStorage.setItem('cartItems', '[]');localStorage.setItem('wishlist', '[]');if (hasRealFirebaseConfig) {firebase.auth().signOut().then(() => {localStorage.removeItem('ganeshStore_user');window.location.reload();}).catch(err => console.error("Error signing out:", err));} else {localStorage.removeItem('ganeshStore_user_mock');window.location.reload();}}function generateCustomerId(email) {let hash = 0;for (let i = 0; i < email.length; i++) {hash = email.charCodeAt(i) + ((hash << 5) - hash);}const id = Math.abs(hash % 90000) + 10000;return `GS-${id}`;}function getRegisteredUsers() {const users = localStorage.getItem('ganeshStore_registered_users');return users ? JSON.parse(users) : [];}function saveRegisteredUser(user) {const users = getRegisteredUsers();users.push(user);localStorage.setItem('ganeshStore_registered_users', JSON.stringify(users));}function getMockUser() {const user = localStorage.getItem('ganeshStore_user_mock');return user ? JSON.parse(user) : null;}function triggerDisplayMessage(type, message) {if (typeof showContactMessage === "function") {showContactMessage(type, message);} else if (typeof showMessage === "function") {showMessage(type, message);} else {alert(message);}}window.handleGoogleSignIn = function() {if (hasRealFirebaseConfig) {const provider = new firebase.auth.GoogleAuthProvider();firebase.auth().signInWithPopup(provider).then((result) => {const user = result.user;const name = user.displayName || user.email.split('@')[0];triggerDisplayMessage('success', 'Google Sign-In successful! Syncing profile...');syncUserProfileWithSupabase(user.uid, name, user.email).then(() => {setTimeout(() => { window.location.reload(); }, 1200);});}).catch((error) => {triggerDisplayMessage('error', error.message);});} else {const randomSuffix = Math.floor(100 + Math.random() * 900);const name = `Google User ${randomSuffix}`;const email = `google.user.${randomSuffix}@gmail.com`;const uid = generateCustomerId(email);const mockUser = { uid, email, displayName: name, photoURL: null };localStorage.setItem('ganeshStore_user_mock', JSON.stringify(mockUser));triggerDisplayMessage('success', 'Google Sign-In simulated! Syncing profile...');syncUserProfileWithSupabase(uid, name, email).then(() => {setTimeout(() => { window.location.reload(); }, 1200);});}};window.handleEmailLogin = function(event) {if (event) event.preventDefault();const emailInput = document.getElementById('c-login-email') || document.getElementById('login-email');const passInput = document.getElementById('c-login-password') || document.getElementById('login-password');if (!emailInput || !passInput) return;const email = emailInput.value.trim();const pass = passInput.value;if (hasRealFirebaseConfig) {firebase.auth().signInWithEmailAndPassword(email, pass).then((userCredential) => {const user = userCredential.user;const name = user.displayName || user.email.split('@')[0];triggerDisplayMessage('success', 'Login successful! Syncing profile...');syncUserProfileWithSupabase(user.uid, name, user.email).then(() => {setTimeout(() => { window.location.reload(); }, 1200);});}).catch((error) => {triggerDisplayMessage('error', error.message);});} else {if (pass.length < 6) {triggerDisplayMessage('error', 'Password must be at least 6 characters long.');return;}const checkUserAndLogin = (existingUser) => {if (!existingUser) {triggerDisplayMessage('error', 'No account found with this email. Please Sign Up to create an account first.');return;}if (existingUser.password && existingUser.password !== pass) {triggerDisplayMessage('error', 'Incorrect password. Please try again.');return;}const name = existingUser.name || email.split('@')[0];const uid = existingUser.id || existingUser.uid || generateCustomerId(email);if (existingUser.two_factor_enabled && existingUser.two_factor_secret) {window.pendingLoginUserId = uid;$('#twofa-login-email-display').text(email);$('#twoFactorModal').css('display', 'flex');$('#twofa-login-code-input').val('').focus();return;}const mockUser = { uid, email, displayName: name, photoURL: null };localStorage.setItem('ganeshStore_user_mock', JSON.stringify(mockUser));triggerDisplayMessage('success', 'Welcome back! Syncing profile...');syncUserProfileWithSupabase(uid, name, email).then(() => {setTimeout(() => { window.location.reload(); }, 1200);});};const users = getRegisteredUsers();const localUser = users.find(u => u.email.toLowerCase() === email.toLowerCase());if (localUser) {checkUserAndLogin(localUser);} else if (window.supabaseClient) {window.supabaseClient.from('profiles').select('*').eq('email', email).then(response => {const profile = response.data && response.data[0];if (profile) {const uid = profile.id;const name = profile.full_name;const newUser = { name, email, password: pass, uid };saveRegisteredUser(newUser);checkUserAndLogin(newUser);} else {checkUserAndLogin(null);}}).catch(err => {console.error("❌ Supabase login check failed:", err);checkUserAndLogin(null);});} else {checkUserAndLogin(null);}}};window.handleEmailSignUp = function(event) {if (event) event.preventDefault();const nameInput = document.getElementById('c-signup-name') || document.getElementById('signup-name');const emailInput = document.getElementById('c-signup-email') || document.getElementById('signup-email');const passInput = document.getElementById('c-signup-password') || document.getElementById('signup-password');if (!nameInput || !emailInput || !passInput) return;const name = nameInput.value.trim();const email = emailInput.value.trim();const pass = passInput.value;if (pass.length < 6) {triggerDisplayMessage('error', 'Password must be at least 6 characters long.');return;}if (hasRealFirebaseConfig) {firebase.auth().createUserWithEmailAndPassword(email, pass).then((userCredential) => {const user = userCredential.user;return user.updateProfile({ displayName: name }).then(() => {user.sendEmailVerification().catch(e => console.error(e));triggerDisplayMessage('success', 'Account created! Syncing profile...');syncUserProfileWithSupabase(user.uid, name, user.email).then(() => {setTimeout(() => { window.location.reload(); }, 1500);});});}).catch((error) => {triggerDisplayMessage('error', error.message);});} else {const users = getRegisteredUsers();const existingUser = users.find(u => u.email.toLowerCase() === email.toLowerCase());if (existingUser) {triggerDisplayMessage('error', 'An account with this email already exists. Please Sign In instead.');return;}const uid = generateCustomerId(email);const mockUser = { uid, email, displayName: name, photoURL: null };saveRegisteredUser({ name, email, password: pass, uid });localStorage.setItem('ganeshStore_user_mock', JSON.stringify(mockUser));triggerDisplayMessage('success', 'Account created successfully! Syncing profile...');syncUserProfileWithSupabase(uid, name, email).then(() => {setTimeout(() => { window.location.reload(); }, 1200);});}};window.handleForgotPassword = function(event) {if (event) event.preventDefault();const emailInput = document.getElementById('c-login-email') || document.getElementById('login-email');if (!emailInput || !emailInput.value) {triggerDisplayMessage('error', 'Please enter your email address in the Sign In form first.');return;}const email = emailInput.value.trim();if (hasRealFirebaseConfig) {firebase.auth().sendPasswordResetEmail(email).then(() => {triggerDisplayMessage('success', 'Password reset email sent! Check your inbox.');}).catch((error) => {triggerDisplayMessage('error', error.message);});} else {triggerDisplayMessage('success', 'Simulation: A password reset email has been sent to ' + email);}};;