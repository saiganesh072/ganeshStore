...
var SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InVyZ2x5dW11Y3h2bnN6eXlyZGl2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzk1MjIxMTksImV4cCI6MjA5NTA5ODExOX0.Fuw5FNHXw_rGoxfeVW8sjCT_DO0lDsOKPJ-wQ2-JV70";var supabaseClient = null;if (SUPABASE_URL && SUPABASE_ANON_KEY) {
...
"use strict";window.activeFilters = {
category: '*',
priceRange: 'all',
color: 'all',
tag: 'all',
...
var windowH = $(window).height()/2;$(window).on('scroll',function(){
...
var wrapMenu = $('.wrap-menu-desktop');if($('.announcement-bar').length > 0) {
...
});var arrowMainMenu = $('.arrow-main-menu-m');for(var i=0; i<arrowMainMenu.length; i++){
...
});var isotopeButton = $('.filter-tope-group button');$(isotopeButton).each(function(){
...
$('.panel-filter').slideToggle(400);if($('.js-show-search').hasClass('show-search')) {
$('.js-show-search').removeClass('show-search');
$('.panel-search').slideUp(400);
}
});$('.js-show-search').on('click',function(){
$(this).toggleClass('show-search');
$('.panel-search').slideToggle(400);if($('.js-show-filter').hasClass('show-filter')) {
...
$(input).val(0);$(item).on('mouseenter', function(){
...
$slick3.empty();var angle1 = $block.attr('data-angle1') || img;
...
var lastModalTrigger = null;window.trapModalFocus = function($modal, $trigger) {
...
}, 120);$modal.off('keydown.trapFocus').on('keydown.trapFocus', function(e) {
if (e.key === 'Tab' || e.keyCode === 9) {
var $focusables = $modal.find(focusableSel).filter(':visible');
if (!$focusables.length) return;
var first = $focusables[0];
var last = $focusables[$focusables.length - 1];if (e.shiftKey) {
...
var $emptyState = $('#wishlist-empty-state');if (!$tableBody.length) return;if (wishlist.length === 0) {
$contentArea.addClass('dis-none');
$emptyState.removeClass('dis-none');
return;
}$emptyState.addClass('dis-none');
$contentArea.removeClass('dis-none');
$tableBody.empty();wishlist.forEach(function(item) {
var rowHtml =
'<tr class="table_row">' +
'  <td class="column-1">' +
...
if ($pB30.length) {
$pB30.find('.flex-w.flex-t.p-b-68').remove();
...
if (!isPDP) return;var pageUrl = encodeURIComponent(window.location.href);
...
var iconClass = $icon.attr('class') || '';if (iconClass.indexOf('fa-facebook') > -1) {
...
if (!isCheckoutPage) return;var profile = null;
try {
...
e.preventDefault();
var $btn = $(this);
if ($btn.hasClass('is-loading')) return;var name = $btn.attr('data-name');
...
$btn.find('.btn-atc-spinner').show();setTimeout(function() {
$btn.removeClass('is-loading');
$btn.find('.btn-atc-text').show();
$btn.find('.btn-atc-spinner').hide();var cart = getCart();
...
setTimeout(function() { $btn.removeClass('atc-success'); }, 1500);showPremiumToast('<strong>' + name + '</strong> has been added to your cart.', 'cart-success', {
image: img
});
}, 900);
});
...
document.body.removeChild(tempInput);showPremiumToast('Wishlist link successfully copied to clipboard! Share it with friends.', 'success');
});
...
var colorVal = $selects.length > 1 ? $selects.eq(1).val() : 'Default';if (!sizeVal || sizeVal.indexOf('Choose') !== -1) sizeVal = 'Size M';
...
$(document).trigger('cartUpdated');setTimeout(function() {
...
e.preventDefault();
var $btn = $(this);
if ($btn.hasClass('is-loading')) return;var $couponInput = $('input[name="coupon"]');
var couponCode = $couponInput.val().trim().toUpperCase();if (!couponCode) {
if (typeof swal === 'function') {
swal("Empty Code", "Please enter a coupon code first.", "warning");
}
return;
}$btn.addClass('is-loading');setTimeout(function() {
$btn.removeClass('is-loading');
var cart = getCart();
var subtotal = 0;
...
e.preventDefault();
var $btn = $(this);
if ($btn.hasClass('is-loading')) return;var $table = $('.table-shopping-cart');
$btn.addClass('is-loading');
$table.addClass('refreshing');setTimeout(function() {
$btn.removeClass('is-loading');
...
if (!$cartWrap.length) return;$cartWrap.empty();
if (cart.length === 0) {
...
var $table = $('.table-shopping-cart');if (!$form.length) return;if (cart.length === 0) {
$form.addClass('dis-none');
$emptyState.removeClass('dis-none');
return;
}$emptyState.addClass('dis-none');
$form.removeClass('dis-none');
$table.find('.table_row').remove();cart.forEach(function(item, index) {
var priceNum = parseFloat(item.price.replace(/[^\d.]/g, '')) || 0;
...
$('.js-cart-grand-total').text('$ ' + finalTotal.toFixed(2));var $totals = $('.mtext-110');
if ($totals.length >= 2) {
...
var missingAttribute = "";$row.find('select').each(function() {
var $select = $(this);
...
}, 400);showPremiumToast('<strong>' + name + '</strong> moved to your Wishlist! ❤️', 'wishlist-added');
}
});
...
if (!profile) return;var cart = JSON.parse(localStorage.getItem('cartItems')) || [];
window.supabaseClient
.from('carts')
.delete()
...
if (!profile) return;var wishlist = JSON.parse(localStorage.getItem('wishlist')) || [];
window.supabaseClient
.from('wishlists')
.delete()
.eq('user_id', profile.id)
...
$originalContainer.before($newLabel);
$originalContainer.parent().removeClass('size-204 respon6-next');if (isSize) {
...
} catch(e) {}if (savedProfile && (savedProfile.first_name || savedProfile.name || savedProfile.email)) {
...
var zip = savedProfile.zip || savedProfile.postcode || '';if (fName) $('input[name="firstname"]').val(fName).addClass('field-valid');
...
if (zip) $('input[name="postcode"]').val(zip).addClass('field-valid');$('#savedProfileName').text((fName + ' ' + lName).trim() || email);
$('#savedProfileBanner').show();
}$('#btnEditSavedInfo').on('click', function() {
$('#savedProfileBanner').slideUp(200);
$('input[name="firstname"]').focus();
});
var activeCouponDiscount = parseInt(sessionStorage.getItem('activeDiscountPercentage')) || 0;
...
'</div>';$summaryList.append(itemHtml);
...
$dropdown.empty();var filtered = addressSuggestions.filter(function(addr) {
...
$(this).addClass('active-tab');$('.payment-panel').removeClass('active-panel');
...
var isValid = true;
var errorMsg = '';
$input.closest('.input-validate-container, .col-sm-6, .col-sm-4, .col-md-6, .m-b-20, .m-b-15').find('.checkout-field-error').remove();if (isRequired && !val) {
isValid = false;
...
var code = $input.val().trim().toUpperCase();if (!code) {
...
if (!$('input[name="postcode"]').val()) $('input[name="postcode"]').val('97477');showPremiumToast('Authenticating with ' + provider + '...', 'info');
...
$('#trk-est-delivery').text(est);var milestones = [
...
var randomSize = Math.floor(Math.random() * 8) + 6;$particle.css({
...
var effectivePayment = window.expressPaymentProvider || activeTab.toUpperCase();if (!window.expressPaymentProvider) {
if (activeTab === 'card') {
...
var taxNum = 0.00;
var totalNum = Math.max(0, subtotalNum - discountNum) + shippingFee + taxNum;var emailVal = $('input[name="checkout-email"]').val().trim();
...
$('#receiptTotal').text('$ ' + totalNum.toFixed(2));$('#receiptContainer').show();
var history = [];
try {
...
}})(jQuery);window.quickApplyCoupon = function(code) {
...
if (!cleanCode) return;var discountPercent = 0;
var discountFixed = 0;if (cleanCode === 'SAVE10') {
discountPercent = 10;
...
sessionStorage.setItem('activeCouponDiscountFixed', discountFixed);var tagHtml = '<div class="applied-coupon-tag">' +
'<span>🏷️ ' + cleanCode + ' Applied</span>' +
'<span class="remove-coupon-btn" onclick="removeAppliedCoupon()">&times;</span>' +
'</div>';
$('#applied-coupon-tag-container').html(tagHtml);if (typeof showToast === 'function') {
...
var $bar = $('#shipping-bar-fill');if (!$box.length) return;if (subtotal >= threshold) {
...
if (!$addCartBtn.length) return;var pName = $('.js-name-detail').text().trim() || 'Product';
var pPrice = $('.mtext-106').first().text().trim() || '$58.79';
var pImg = $('.wrap-pic-w img').first().attr('src') || 'images/product-01.jpg';var barHtml = '<div class="sticky-pdp-bar" id="sticky-pdp-bar">' +
...
'</div>' +
'</div>';$('body').append(barHtml);var $bar = $('#sticky-pdp-bar');
var btnTop = $addCartBtn.offset().top + 100;$(window).on('scroll', function() {
...
if (!$imgElement || !$imgElement.length) return;var $cartIcon = $('.js-show-cart').first();
if (!$cartIcon.length) return;var imgPos = $imgElement.offset();
var cartPos = $cartIcon.offset();var $clone = $imgElement.clone()
...
e.preventDefault();
var $btn = $(this);
var $parentPDP = $btn.closest('.p-r-50, .p-r-0-lg, .modal-content');if ($parentPDP.length) {
...
if (!$('.wrap-num-product').length) return;var sizeGuideHtml = '<div class="size-guide-modal-box modal-dialog-centered" id="size-guide-modal-content">' +
...
if (!pName) return;var pPrice = $('.mtext-106').first().text().trim() || '$58.79';
var pImg = $('.wrap-pic-w img').first().attr('src') || 'images/product-01.jpg';
var pUrl = window.location.href;var recentList = [];
try {
...
if (recentList.length > 6) recentList = recentList.slice(0, 6);localStorage.setItem('ganeshRecentlyViewed', JSON.stringify(recentList));
...
if (recentList.length < 2) return;var $container = $('#recently-viewed-ribbon');
if (!$container.length) return;var itemsHtml = '';
...
if (!$priceContainer.length) return;var pName = $('.js-name-detail').text().trim() || 'Product';
var hash = 0;
...
'</div>';$('body').append(progressBtnHtml);var $btn = $('#scroll-progress-btn');
var $circle = $('.scroll-progress-circle');
var maxDash = 145;$('.btn-back-to-top').remove();$(window).on('scroll', function() {
var scrollTop = $(window).scrollTop();
var docHeight = $(document).height() - $(window).height();
var progress = scrollTop / docHeight;
var dashoffset = maxDash - (progress * maxDash);$circle.css('stroke-dashoffset', dashoffset);if (scrollTop > 300) {
...
var cleanQ = (query || '').trim().toLowerCase();if (!cleanQ) {
$clearBtn.hide();
$trending.fadeIn(200);
$resultsWrap.hide();
$empty.hide();
...
var remaining = (threshold - subtotal).toFixed(2);var $progressContainers = $('.shipping-progress-container');if (!$progressContainers.length) {
...
}, 450);if (typeof window.updateFreeShippingProgressBar === 'function') {
window.updateFreeShippingProgressBar();
}
});
}function initUniversalReviewSystem() {
var $reviewForm = $('#reviews form, .tab-pane#reviews form');
if (!$reviewForm.length) return;var productName = $('.js-name-detail').text().trim() || $('h4.mtext-105').text().trim() || document.title.split('-')[0].trim();
...
var email = $form.find('input[name="email"], #email').val().trim() || 'customer@ganeshstore.com';if (!comment) {
...
var initials = (name.substring(0, 2).toUpperCase()).replace(/[^A-Z0-9]/g, '');var reviewCardHtml =
...
'  </div>' +
'</div>';var $revContainer = $form.closest('.p-b-30');
...
$form.find('.wrap-rating i').removeClass('zmdi-star').addClass('zmdi-star-outline');if (typeof showPremiumToast === 'function') {
...
var email = $input.val().trim();if (!email || email.indexOf('@') === -1) {
if (typeof showPremiumToast === 'function') {
...
var name = $form.find('input[name="name"]').val() ? $form.find('input[name="name"]').val().trim() : (email ? email.split('@')[0] : 'Guest');if (!email || !msg) {
if (typeof showPremiumToast === 'function') {
...
var $cartWrap = $('.header-cart-wrapitem');
if ($cartWrap.length === 0) return;if (!cart || cart.length === 0) {
$cartWrap.html(
...
var total = 0;cart.forEach(function(item) {
var priceNum = parseFloat(String(item.price).replace(/[^0-9.]/g, '')) || 0;
var qty = parseInt(item.quantity, 10) || 1;
total += priceNum * qty;var safeName = window.DOMSanitizer ? window.DOMSanitizer.escapeHTML(item.name) : item.name;
...
var imgSrc = item.image || 'images/item-cart-01.jpg';html +=
...
$('.header-cart-total').text('Total: $' + total.toFixed(2));if (typeof window.updateFreeShippingProgressBar === 'function') {
window.updateFreeShippingProgressBar();
}
}
...
if ($detailSec.length === 0 || $mainAtcBtn.length === 0) return;var prodTitle = $('.js-name-detail').first().text().trim() || 'Luxury Product';
var prodPrice = $detailSec.find('.mtext-106').first().text().trim() || '$0.00';
var prodImg = $('.item-slick3 img').first().attr('src') || 'images/product-01.jpg';if ($('#stickyMobileAtc').length === 0) {
var stickyHtml =
...
}var $stickyBar = $('#stickyMobileAtc');$(document).on('click', '#btnStickyMobileAtc', function(e) {
e.preventDefault();
...
var windowBottom = scrollTop + $(window).height();if (scrollTop > atcOffsetTop && windowBottom < footerTop + 60) {
...
window.showProductGridSkeletons = showProductGridSkeletons;function initStockUrgencyEngine() {
var $detailSec = $('.sec-product-detail');
if ($detailSec.length === 0) return;var prodName = $('.js-name-detail').first().text().trim();
if (!prodName || !window.BackendService) return;var prod = window.BackendService.products.getProductByName(prodName);
var stock = (prod && typeof prod.stock_quantity === 'number') ? prod.stock_quantity : ((prod && typeof prod.stock === 'number') ? prod.stock : 4);if ($('.stock-urgency-badge').length === 0) {
var badgeHtml = '';
if (stock > 0 && stock <= 5) {
...
var $grid = $('.isotope-grid');
if ($grid.length === 0) return;var pageSize = 16;
var currentPage = 1;if ($('#catalogPagination').length === 0) {
$grid.after('<div id="catalogPagination" class="catalog-pagination-container"></div>');
}var $paginationContainer = $('#catalogPagination');function getMatchingItems() {
...
var totalPages = Math.ceil(totalItems / pageSize) || 1;if (currentPage > totalPages) currentPage = totalPages;
...
var endDisplay = Math.min(endIndex, totalItems);var html =
...
html += '<button type="button" class="pagination-btn pagination-prev" data-page="' + (currentPage - 1) + '" ' + (currentPage === 1 ? 'disabled aria-disabled="true"' : '') + ' aria-label="Previous page"><i class="zmdi zmdi-chevron-left"></i></button>';for (var p = 1; p <= totalPages; p++) {
...
html += '<button type="button" class="pagination-btn pagination-next" data-page="' + (currentPage + 1) + '" ' + (currentPage === totalPages ? 'disabled aria-disabled="true"' : '') + ' aria-label="Next page"><i class="zmdi zmdi-chevron-right"></i></button>';html += '</div>';$paginationContainer.html(html);
}
$(document).on('click', '.pagination-btn', function(e) {
e.preventDefault();
var targetPage = parseInt($(this).attr('data-page'), 10);
if (!targetPage || $(this).prop('disabled')) return;currentPage = targetPage;
renderPagination();if ($grid.offset()) {
$('html, body').animate({
...
var currentUrl = window.location.pathname.split('/').pop() || 'product-detail.html';if (prodTitle) {
var recentlyViewed = [];
try {
...
if (recentlyViewed.length > 6) recentlyViewed = recentlyViewed.slice(0, 6);try {
...
var safeImg = item.image || 'images/product-01.jpg';cardsHtml +=
'<div class="col-sm-6 col-md-4 col-lg-3 p-b-35">' +
'  <div class="block2">' +
'    <div class="block2-pic hov-img0">' +
